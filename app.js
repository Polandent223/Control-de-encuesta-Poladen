...
function renderStep(){const box=$('#clientContent');if(!box)return;const qs=data.questions.filter(q=>q.active).sort((a,b)=>a.order-b.order),branches=data.branches.filter(b=>b.active),channels=data.channels.filter(c=>c.active),emps=data.employees.filter(e=>e.active&&(!form.branchId||e.branchId===form.branchId)&&(!form.channelId||(e.channelIds||[]).includes(form.channelId)));
 if(step===0)box.innerHTML=`<div class="welcomeHero"><span class="heroBadge">Encuesta rápida · 30 segundos</span><h2>¡Ayúdanos a crecer!</h2><p>Queremos saber cómo fue la atención de nuestro equipo de ventas.</p></div><div class="field"><label>¿En cuál sede recibiste atención?</label><div class="branchChoices">${branches.map(b=>`<button type="button" class="branchChoice ${form.branchId===b.id?'selected':''}" data-branch="${b.id}">📍 ${b.name}</button>`).join('')}</div></div><button class="btn" id="next">Continuar</button>`;
 if(step===1)box.innerHTML=`<h2 class="stepTitle">¿Por cuál medio te atendimos?</h2><div class="channelChoices">${channels.map(c=>`<button class="channelChoice ${form.channelId===c.id?'selected':''}" data-channel="${c.id}"><span>${c.icon}</span><b>${c.name}</b></button>`).join('')}</div><div class="stepActions"><button class="btn secondary" id="back">Atrás</button><button class="btn" id="next">Continuar</button></div>`;
...
function bindClient(){
 $$('.branchChoice').forEach(b=>b.onclick=()=>{form.branchId=b.dataset.branch;form.channelId='';form.employeeIds=[];renderStep()});
 ...
 $$('.modeChoice').forEach(x=>x.onclick=()=>{form.evaluationMode=x.dataset.mode;renderStep()});
 $('#next')&&($('#next').onclick=()=>{if(step===0){if(!form.branchId)return alert('Selecciona una sede');step=1}else if(step===1){if(!form.channelId)return alert('Selecciona un canal');step=2}else if(step===2){if(!form.employeeIds?.length)return alert('Selecciona al menos una persona');form.evaluationMode=form.employeeIds.length>1?(form.evaluationMode||'team'):'team';step=form.employeeIds.length>1?3:3}else if(step===3&&form.employeeIds.length>1){if(!form.evaluationMode)return alert('Selecciona cómo deseas evaluar');step=4}renderStep()});
 ...
 const anon=$('#anonymous');if(anon)anon.onchange=()=>{form.anonymous=anon.checked;renderStep()};
 $('#send')&&($('#send').onclick=async()=>{const qs=data.questions.filter(q=>q.active);$$('[data-qinput]').forEach(i=>form.answers[i.dataset.qinput]=i.value);const missing=qs.filter(q=>q.required&&!form.answers[q.id]);if(missing.length)return alert('Responde las preguntas obligatorias');if(form.evaluationMode==='individual'&&form.employeeIds.some(id=>!(form.employeeRatings||{})[id]))return alert('Califica a cada persona seleccionada');const btn=$('#send');btn.disabled=true;btn.textContent='Enviando...';form.badComment=$('#badComment')?.value||'';form.anonymous=$('#anonymous')?.checked!==false;form.clientName=form.anonymous?'':($('#cName')?.value||'');form.clientPhone=form.anonymous?'':($('#cPhone')?.value||'');const meta=await getClientMeta();const ratingQ=qs.find(q=>q.type==='rating'),commentQ=qs.find(q=>q.type==='text');const rating=form.evaluationMode==='individual'?Math.round(form.employeeIds.reduce((a,id)=>a+Number(form.employeeRatings[id]||0),0)/form.employeeIds.length):(form.rating||form.answers[ratingQ?.id]||'');const survey={id:uid(),date:new Date().toISOString(),linkId:form.linkId||'',anonymous:form.anonymous,clientName:form.clientName,clientPhone:form.clientPhone,branchId:form.branchId,channelId:form.channelId,employeeIds:[...form.employeeIds],employeeId:form.employeeIds[0],evaluationMode:form.evaluationMode||'team',employeeRatings:form.employeeRatings||{},rating,comment:form.answers[commentQ?.id]||'',answers:{...form.answers},badOther:form.badOther||'no',badEmployeeId:form.badEmployeeId||'',badComment:form.badComment||'',...meta};survey.abuseToday=abuseCount(survey);data.surveys.push(survey);if(form.linkId)markLinkCompleted(form.linkId,survey.id);save();step=99;renderStep()})
}
