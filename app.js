...
function bindClient(){ const anon=$('#anonymous'); if(anon){anon.onchange=()=>{form.anonymous=anon.checked; if(form.anonymous){form.clientName='';form.clientPhone='';} renderStep()}} $('#next')&&($('#next').onclick=()=>{if(step===0){form.anonymous=!!$('#anonymous')?.checked;form.clientName=form.anonymous?'':($('#cName')?.value||'');form.clientPhone=form.anonymous?'':($('#cPhone')?.value||'');form.city=$('#cCity')?.value||'';step=1}else if(step===1){if(!form.employeeId)return alert('Selecciona quién te atendió');step=2}renderStep()}); $('#back')&&($('#back').onclick=()=>{step--;renderStep()}); $$('.empChoice').forEach(x=>x.onclick=()=>{form.employeeId=x.dataset.emp;renderStep()}); $$('.rate').forEach(b=>b.onclick=e=>{e.preventDefault();form.answers[b.parentElement.dataset.q]=b.dataset.rate; form.rating=b.dataset.rate; renderStep()}); $$('[data-qinput]').forEach(i=>i.onchange=()=>form.answers[i.dataset.qinput]=i.value); $('#badOther')&&($('#badOther').onchange=()=>$('#badBox').classList.toggle('hidden',$('#badOther').value!=='si')); $('#send')&&($('#send').onclick=async()=>{ $('#send').disabled=true; $('#send').textContent='Enviando...'; $$('[data-qinput]').forEach(i=>form.answers[i.dataset.qinput]=i.value); const ratingQ=data.questions.find(q=>q.type==='rating'&&q.active); const commentQ=data.questions.find(q=>q.type==='text'&&q.active); const meta=await getClientMeta(); const survey={id:uid(),date:new Date().toISOString(),anonymous:!!form.anonymous,clientName:form.anonymous?'':form.clientName,clientPhone:form.anonymous?'':form.clientPhone,city:form.city,employeeId:form.employeeId,rating:form.rating||form.answers[ratingQ?.id]||'',comment:form.answers[commentQ?.id]||'',answers:form.answers,badOther:$('#badOther').value,badEmployeeId:$('#badEmp')?.value||'',badComment:$('#badComment')?.value||'',...meta}; survey.abuseToday=abuseCount(survey); data.surveys.push(survey); save(); step=3; renderStep()})}
