...
function normalizeData(d){
  ...
  d.surveys=d.surveys.map(s=>({...s,id:s.id||uid(),date:s.date||new Date().toISOString(),answers:s.answers||{},badOther:s.badOther||'no',badEmployeeId:s.badEmployeeId||'',badComment:s.badComment||'',followStatus:s.followStatus||'Pendiente',followNote:s.followNote||''}));
  return d;
}
function defaultSeed(saveIt=true){ const seed={pin:'2233',employees:[{id:uid(),name:'Yexi',role:'Ventas al mayor',active:true,photo:emptyPhoto},{id:uid(),name:'Tibisay',role:'Área de ventas',active:true,photo:emptyPhoto},{id:uid(),name:'Génesis',role:'Área de ventas',active:true,photo:emptyPhoto}],questions:[{id:uid(),text:'¿Cómo califica la atención recibida?',type:'rating',required:true,active:true,order:1},{id:uid(),text:'¿Fue atendido con amabilidad y respeto?',type:'yesno',required:true,active:true,order:2},{id:uid(),text:'¿Le explicaron bien el producto o servicio?',type:'yesno',required:true,active:true,order:3},{id:uid(),text:'¿Recomendaría Poladent Casa Dental?',type:'yesno',required:true,active:true,order:4},{id:uid(),text:'Comentario adicional',type:'text',required:false,active:true,order:5}],settings:{reportQuestion:{text:'¿Alguna otra persona te atendió de forma inadecuada o te hizo sentir incómodo/a?',active:true}},surveys:[]}; if(saveIt) localStorage.setItem(DB,JSON.stringify(seed)); return seed}
